...
- (void)updateUI {
    ...
    UIView *viewPreView = [[UIView alloc]initWithFrame:CGRectMake(50, 50, kSize.width - 100, kSize.width - 100)];
    viewPreView.backgroundColor = [UIColor cyanColor];
    [self.view addSubview:viewPreView];
    self.viewPreview = viewPreView;
    
    UILabel *lblStatus = [[UILabel alloc]init];
    lblStatus.center = self.viewPreview.center;
    lblStatus.frame = self.viewPreview.frame;
    lblStatus.text = @"lblStatus";
    lblStatus.textAlignment = NSTextAlignmentCenter;
    [self.view addSubview:lblStatus];
    self.lblStatus = lblStatus;
    
    UIButton *startBtn = [[UIButton alloc]initWithFrame:CGRectMake(100, CGRectGetMaxY(self.viewPreview.frame) + 30, kSize.width-200, 30)];
    [startBtn addTarget:self action:@selector(startStopReading:) forControlEvents:UIControlEventTouchUpInside];
